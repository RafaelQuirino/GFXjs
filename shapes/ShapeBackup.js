__gfx_shapes.createShape = function(obj) {
    ...
    obj.Shape = function() {
        ...
        this._color = new __gfx_aux.Color(0,0,0);
        this._renderBorder = false;
        this._borderColor = new __gfx_aux.Color(0,0,0);
        this._thickness = 1;
    ...
    obj.Shape.prototype = {
        ...
        style: function() {
            ...
            else if (arguments.length == 1) this._style = arguments[0];
        },

        color: function() {
            if (arguments.length == 0) return this._color;
            else if (arguments.length == 3) {
                this._color = new __gfx_aux.Color(arguments[0], arguments[1], arguments[2]);
            }
        },
        ...
        borderColor: function() {
            if (arguments.length == 0) return this._borderColor;
                ...
                var g = arguments[1];
                var b = arguments[2];
                this._borderColor = new __gfx_aux.Color(r,g,b);
            }
        },
        ...
        */
        ...
        incline: function(angle) {
            if (angle == 0) {
                return;
            } else if (Math.abs(angle) < 90) {
                var anglerad = (Math.PI * angle) / 180;
                var pt1 = {x: this._points[0].x(), y: this._points[0].y()};
                var pt2 = {x: this._points[1].x(), y: this._points[1].y()};

                var d = __gfx_helpers.distance(pt1, pt2);
                var sin = Math.sin(anglerad);

                //These are, in fact, the max dx and max dy for points in the bounding box right line
                var dx = (d - Math.sqrt((Math.pow(d,2) - Math.pow(sin * d,2)))) * -1;
                var dy = angle > 0 ? -1 : 1;
                dy *= sin * d;

                this._boundingBoxCenter.x = this._boundingBoxCenter.x + dx;
                this._boundingBoxCenter.y = this._boundingBoxCenter.y + dy;
                
                for (var i = 0; i < this._boundingBox.length; i++) {
                    if (i != 0 && i != 3) {
                        this._boundingBox[i].x = this._boundingBox[i].x + dx;
                        this._boundingBox[i].y = this._boundingBox[i].y + dy;
                    }
                }

                var lines = this.getBoundaryPoints();
                for (var i = 0; i < this._points.length; i++) {
                    //Test if points are on the left most line of the bounding box
                    var flag = true;
                    for (var j = 0; j < lines[3].length; j++) {
                        if (this._points[i].x() == lines[3][j].x && 
                            this._points[i].y() == lines[3][j].y) {
                            flag = false;
                        }
                    }
                    if (flag) {
                        this._points[i].x(this._points[i].x() + dx);
                        this._points[i].y(this._points[i].y() + dy);
                    }
                }
            } else {
